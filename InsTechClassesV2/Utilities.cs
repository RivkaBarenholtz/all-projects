...
namespace InsTechClassesV2
{
    public static class Utilities
    {
        ...
        //}
        public async static Task<Vendor?> GetVendor(string param)
        {
            //var subdomain = GetSubdomain(host);
            var vendors = await GetVendorListAsync();
            return vendors?.FirstOrDefault(v =>( v.subdomain == param || v.SecondaryDomain == param) && !v.Exclude );
        ...
        public async static Task<List<Vendor>?> GetVendorListAsync()
        {
            ...
            List<Vendor>? vendors =await DynamoDatabaseTransactions.GetEntityItemsFromDynamoAsync<Vendor>("VENDOR#INS", "VENDOR#");
            return vendors;
